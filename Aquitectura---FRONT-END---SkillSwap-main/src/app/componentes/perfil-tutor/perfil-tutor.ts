...
export class PerfilTutorComponent implements OnInit {
  ...
  readonly inicial = computed(() => this.tutor()?.nombreCompleto?.charAt(0).toUpperCase() ?? '?');
  ...
  get habilidadSeleccionada(): number | null {
    return this._habilidadSeleccionada();
  }
  set habilidadSeleccionada(v: number | null) {
    this._habilidadSeleccionada.set(v);
  }
  ...
  ngOnInit(): void {
    ...
    forkJoin({
      ...
      tutorHabs: this.usuarioHabilidadSvc
        .getByUsuario(id)
      ...
      const instMap = new Map(
        (instituciones as Institucion[]).map((i) => [i.idInstitucion, i.nombre]),
      );
      const tutorObj = tutor as Usuario | null;
      this.institucionNombre.set(instMap.get(tutorObj?.idInstitucion ?? 0) ?? 'Sin institución');
      ...
      forkJoin(
        ...
        const nameMap = new Map(
          uniqueIds.map((uid, i) => [
            uid,
            (usuarios[i] as Usuario | null)?.nombreCompleto ?? `Estudiante #${uid}`,
          ]),
        );
        this.resenas.set(
          resenasList.map((r) => ({
            resena: r,
            evaluadorNombre:
              nameMap.get(r.idUsuarioEvaluador) ?? `Estudiante #${r.idUsuarioEvaluador}`,
          })),
        );
        this.cargando.set(false);
      });
